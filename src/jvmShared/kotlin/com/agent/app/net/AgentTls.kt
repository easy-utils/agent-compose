...
// The private ingress CA (same cert the Flutter app and appstore-kmp bundle).
// Android trusts it through res/xml/network_security_config.xml (@raw/agent_ca)
// — no code-level TLS customization, per the official Android security-config
// guidance. The desktop JVM has no such mechanism, so it installs the CA into
// OkHttp's trust store itself (see DesktopAgentTls.kt).
const val AGENT_CA_PEM: String = """
-----BEGIN CERTIFICATE-----
...
/**
 * The OkHttp client used for every agent RPC.
 *
 * Android: a stock client — the bundled CA is trusted by Android's default
 * `X509TrustManager` because of the network security config. Do NOT install a
 * custom TrustManager here: on Android OkHttp resolves to the Conscrypt-backed
 * `okhttp-android`, whose certificate-chain cleaner / trust-root index assume
 * the platform TrustManager and are not built to wrap arbitrary ones.
 *
 * Desktop: trusts the system CAs plus the bundled private CA (implemented in
 * the desktop actual).
 */
expect fun buildAgentOkHttpClient(): OkHttpClient
